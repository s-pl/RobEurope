...
const CompetitionChat = ({ competitionId }) => {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('competition_message', (message) => {
    ...
    socketRef.current.on('competition_message_reaction_added', ({ messageId, reaction }) => {
    ...
    socketRef.current.on('competition_message_reaction_removed', ({ messageId, userId, emoji }) => {
      setMessages((prev) => prev.map(msg => {
        if (msg.id === messageId) {
          const reactions = msg.Reactions || [];
  ...
  const handleReaction = async (messageId, emoji) => {
    try {
      ...
      if (existingReaction) {
        await api(`/competitions/${competitionId}/messages/${messageId}/reactions`, {
          method: 'DELETE',
          body: { emoji }
        });
      } else {
  ...
  return (
    <div className="flex flex-col h-[600px] border rounded-lg bg-white dark:bg-slate-950 dark:border-slate-800">
      <div className="p-4 border-b dark:border-slate-800 flex justify-between items-center">
        <div>
          <h3 className="font-semibold text-lg">{t('competition.chat.title') || 'Chat de Competición'}</h3>
      ...
      <div className="p-4 border-t dark:border-slate-800">
        ...
        <form onSubmit={handleSendMessage} className="flex gap-2">
          ...
          <Input
            ...
            placeholder={t('competition.chat.placeholder') || 'Escribe un mensaje...'}
            className="flex-1"
          />
