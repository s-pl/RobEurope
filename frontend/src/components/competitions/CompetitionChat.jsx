...
const getUserReaction = (reactions = [], userId) => {
  ...
  const match = reactions.find((reaction) => reaction.user_id === userId);
...
const CompetitionChat = ({ competitionId }) => {
  ...
  useEffect(() => {
    ...
    const handleReactionUpdate = (payload) => {
      if (!payload || typeof payload.messageId === 'undefined') return;
      ...
      <ScrollArea className="flex-1 p-4">
        ...
        <div className="space-y-4">
          ...
          {messages.map((msg) => {
            ...
            const userReaction = getUserReaction(msg.reactions || [], user.id);
            ...
            return (
              <div key={msg.id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`mt-2 flex flex-wrap gap-2 ${isMe ? 'justify-end' : ''}`}>
                    {Object.entries(groupedReactions).map(([emoji, userIds]) => (
                      <button
                        key={emoji}
                        type="button"
                        onClick={() => handleSelectReaction(msg.id, userReaction === emoji ? null : emoji)}
                        className={`px-2 py-0.5 rounded-full border text-xs flex items-center gap-1 transition ${
                          userReaction === emoji
                            ? 'bg-blue-600 text-white border-blue-600'
                            : 'bg-white/80 border-slate-200 text-slate-600 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300'
                        }`}
                      >
                        <span>{emoji}</span>
                    ...
                    {canReact && (
                      <button
                        type="button"
                        className="px-2 py-0.5 rounded-full border border-dashed text-xs flex items-center gap-1 text-slate-500 hover:border-blue-400 hover:text-blue-600"
                ...
                {reactionTarget === msg.id && canReact && (
                  <div className={`mt-2 inline-flex flex-wrap items-center gap-1 rounded-full border border-slate-200 dark:border-slate-700 px-3 py-1 bg-white dark:bg-slate-900 shadow-sm ${isMe ? 'self-end' : ''}`}>
