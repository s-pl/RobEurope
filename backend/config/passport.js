...
// passport.serializeUser / deserializeUser not needed in JWT mode,
// but passport middleware still requires them if passport.session() is called.
...
const handleSocialLogin = async (req, accessToken, refreshToken, profile, done, providerField) => {
  try {
    ...
    if (existingUser) username = `${username}_${uuidv4().substring(0, 4)}`;
...
const callbackURL = (path) =>
  process.env.API_BASE_URL
    ...
    : `https://api.robeurope.samuelponce.es${path}`;
